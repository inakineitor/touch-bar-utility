...
export default {
  ...
  config : {
    buttons: {
      title: 'Elements',
      description: 'Here you must specify the elements that you want appearing on your Touch Bar. If you don\'t know how, follow the instructions below (or in the README.md file).',
      type: 'string',
...
function clog(message) {
  console.log({package: 'touch-bar-utility', message: message});
}

function cdbug(message) {
  console.debug({package: 'touch-bar-utility', message: message});
}

function configureButtons(configString, callback) {
  const {
    TouchBarButton,
    TouchBarColorPicker,
    TouchBarGroup,
    ...
    itemsConfig.forEach(function(element) {
      ...
      switch (element.type) {
        case 'button':
          ...
          touchBarElement = new TouchBarButton({
            ...
            icon: element.pathOfIcon
              ? NativeImage.createFromPath(element.pathOfIcon)
              : undefined,
            iconPosition: element.iconPosition,
            click: element.clickDispatchAction
              ? dispatchAction(element.clickDispatchAction)
        ...
        case 'color-picker':
          ...
          touchBarElement = new TouchBarColorPicker({availableColors: element.availableColors, selectedColor: element.selectedColor, change: element.change});
          break;
        ...
        case 'group':
          ...
          this.configureButtons(element.items, function(err, items) {
            if (err) {
              ...
              touchBarElement = new TouchBarGroup({items: new TouchBar(items)});
            }
          });
          break;
        ...
        case 'label':
          ...
          touchBarElement = new TouchBarLabel({label: element.label, textColor: element.textColor});
          break;
          ...
          this.configureButtons(element.items, function(err, items) {
            if (err) {
              ...
              touchBarElement = new TouchBarPopover({
                label: element.label,
                icon: element.pathOfIcon ? NativeImage.createFromPath(element.pathOfIcon) : undefined,
        ...
        case 'slider':
          ...
          touchBarElement = new TouchBarSlider({label: element.label, value: element.value, minValue: element.minValue, maxValue: element.maxValue, change: element.change});
          break;
        ...
        case 'spacer':
          ...
          touchBarElement = new TouchBarSpacer({size: element.size});
          break;
