...
export default {
  ...
  config: {
    ...
    buttons: {
      ...
      type: 'string',
      default: '[{"type":"label", "label":"touch-bar-utility rocks! 🎸🤘"}, {"type": "button", "label": "💅", "clickDispatchAction": "atom-beautify:beautify-editor", "backgroundColor":"#b355d6"}, {"type": "button", "label": "//", "clickDispatchAction": "editor:toggle-line-comments", "backgroundColor":"#4899a8"}]'
    }
  },
  ...
  activate(state) {
    ...
    if (TouchBar) {
      atom.config.observe('touch-bar-utility.active', function(value) {
        if(value){
          configureButtons(atom.config.get('touch-bar-utility.buttons'), function(err, items) {
            if (err) {
              ...
              atom.getCurrentWindow().setTouchBar(new TouchBar(items));
            }
          });
        } else {
      ...
      switch (element.type) {
        case 'button':
          ...
          if (element.dispatchActionTarget && !(element.dispatchActionTarget == 'editor' || element.dispatchActionTarget == 'workspace'))
            return callback(new Error("The button's dispatchActionTarget must have a avalue of 'editor' or 'workspace'"));
...
function dispatchAction(compositeEvent, eventTarget) {
  ...
  if (!PackageManager.getAvailablePackageNames().includes(atomPackage) && atomPackage != 'editor') {
  ...
  } else if (PackageManager.isPackageDisabled(atomPackage) && atomPackage != 'editor') {
    ...
    if ((!PackageManager.isPackageLoaded(atomPackage) || !PackageManager.isPackageActive(atomPackage)) && atomPackage != 'editor') {
